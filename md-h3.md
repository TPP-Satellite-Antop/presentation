...
> Necesitamos discretizar la Tierra → elegimos hexágonos → usamos H3 → así construimos las direcciones ANTop.

La sección tiene dos partes:
1. **Qué es H3** y por qué hexágonos (slides 1-6).
2. **Cómo se construyen las direcciones ANTop** a partir de H3 (slides 7-11).
...
Puntos clave:
- Una grilla cuadrada proyectada sobre una esfera introduce distorsiones geométricas crecientes (especialmente en los polos).
- Los **hexágonos** tienen propiedades geométricas más favorables:
  - Todos los vecinos están a la **misma distancia** del centro (no ocurre con cuadrados: los vecinos diagonales están más lejos).
  - **Mayor isotopía** en la representación de vecindad.
  - Menor distorsión de área al proyectar sobre superficies curvas.
- Estas propiedades son clave para ANTop: la distancia entre direcciones debe aproximar correctamente la distancia topológica entre satélites.

**Elemento visual sugerido:** Comparativa: celda cuadrada con vecinos a dos distancias distintas (lateral y diagonal resaltados) vs. celda hexagonal con todos los vecinos equidistantes. Mostrar también la deformación en los polos de una grilla cuadrada.
...
- Resolución 0: **122 celdas**. Cada celda se subdivide en ~7 celdas en el nivel siguiente.
...
## Slide 5 — La dimensión prima: solución a la discrepancia IJK
...
## Slide 6 — Origen del hipercubo y partición del espacio

**Contenido:** Por qué se usan múltiples hipercubos y por qué los orígenes son las celdas pentagonales.

Puntos clave:
- Un único sistema de coordenadas IJK para toda la Tierra acumula **distorsiones crecientes**: los pentágonos provocan rotaciones implícitas de los ejes a medida que nos alejamos del origen.
...
- Origen de cada hipercubo: una **celda pentagonal**. ¿Por qué?
  ...
  - Los pentágonos coinciden con los vértices del icosaedro → su coordenada IJK respecto de sí mismos es $(0,0,0)$ → correspondencia directa con el origen del sistema global.

**Elemento visual sugerido:** Imagen de los 12 pentágonos distribuidos sobre la Tierra, cada uno marcado como origen de su hipercubo local. Mostrar cómo dividen la superficie en 12 regiones.

---

## Slide 7 — Construcción de direcciones ANTop: el concepto
...
- Hay 6 movimientos posibles (6 ejes entre IJK + IJK prima), numerados 1 a 6.
...
## Slide 8 — Construcción de direcciones: paso 1 (descomposición en movimientos)
...
**Elemento visual sugerido:** La misma imagen base de la slide 7, ahora con los 3 movimientos dibujados como flechas sobre la grilla. Cada flecha etiquetada con su vector IJK.

---

## Slide 9 — Construcción de direcciones: paso 2 (codificación en bits)
...
## Slide 10 — Construcción de direcciones: paso 3 (compresión en bytes)
